...
function EmploymentProjectionsGauteng() {
  ...
  let employmentData4 = employmentData.cos_emp
  let employmentData5 = employmentData.cow_emp
  ...
  let pop_cityGrowthRate4 = pop_cityGrowthRate.pop_cityGrowthRate4
  let pop_cityGrowthRate5 = pop_cityGrowthRate.pop_cityGrowthRate5
  ...
  let [percentageChange4, setPercentageChange4] = useState(initial_changes)
  let [percentageChange5, setPercentageChange5] = useState(initial_changes)
  ...
  employmentData4.forEach((employment_row) => {
    const { year, sector_id } = employment_row
    const index = percentageChange4.findIndex((perc_change_row) => perc_change_row.year === year)
    employment_row.change = parseFloat(percentageChange4[index][sector_id])
  })

  employmentData4.forEach((employment_row) => {
    const { year } = employment_row
    const index = pop_cityGrowthRate4.findIndex((pop_city_growth_rate_row) => pop_city_growth_rate_row.year === year)
    employment_row.economic_change = parseFloat(pop_cityGrowthRate4[index].change_economic)
    employment_row.predicted_economic_growth = parseFloat(pop_cityGrowthRate4[index].predicted_economic_growth)
  })
 
  // Iterate over each year
  let previousYearJobs4 = {};
  let previousYearGVAPerc4 = {};
  
  for (const year of years) {
    const yearData = employmentData4.filter((entry) => entry.year === year);
    const sectors = [...new Set(yearData.map((entry) => entry.sector_id))];
    ...
    if (year === 2011) {
      ...
      for (const sector of sectors) {
        ...
        for (const entry of sectorData) {
          ...
          previousYearJobs4[sector] = sectorTotalJobs;
          previousYearGVAPerc4[sector] = newGvaPerc;
          entry.total_number_of_jobs = sectorTotalJobs;
        }
      }
    } else {
      for (const sector of sectors) {
        ...
        for (const entry of sectorData) {
          econ = entry.economic_change +entry.predicted_economic_growth
          yearSum += previousYearJobs4[sector] + (previousYearJobs4[sector] * entry.relationship_variable * (econ / 100));
        }
      }
      ...
      for (const sector of sectors) {
        const sectorData = yearData.filter((entry) => entry.sector_id === sector);
        for (const entry of sectorData) {
          const newGvaPerc = previousYearGVAPerc4[sector] + entry.change / 100;
          const sectorTotalJobs = yearSum * newGvaPerc;
  ...
  employmentData3.forEach((employment_row) => {
    const { year, sector_id } = employment_row
    const index = percentageChange5.findIndex((perc_change_row) => perc_change_row.year === year)
    employment_row.change = parseFloat(percentageChange5[index][sector_id])
  })

  employmentData5.forEach((employment_row) => {
    const { year } = employment_row
    const index = pop_cityGrowthRate5.findIndex((pop_city_growth_rate_row) => pop_city_growth_rate_row.year === year)
    employment_row.economic_change = parseFloat(pop_cityGrowthRate5[index].change_economic)
    employment_row.predicted_economic_growth = parseFloat(pop_cityGrowthRate5[index].predicted_economic_growth)
  })
 
  // Iterate over each year
  let previousYearJobs5 = {};
  let previousYearGVAPerc5 = {};
  
  for (const year of years) {
    const yearData = employmentData5.filter((entry) => entry.year === year);
    const sectors = [...new Set(yearData.map((entry) => entry.sector_id))];
    ...
    if (year === 2011) {
      ...
      for (const sector of sectors) {
        ...
        for (const entry of sectorData) {
          ...
          previousYearJobs5[sector] = sectorTotalJobs;
          previousYearGVAPerc5[sector] = newGvaPerc;
          entry.total_number_of_jobs = sectorTotalJobs;
        }
      }
    } else {
      for (const sector of sectors) {
        ...
        for (const entry of sectorData) {
          econ = entry.economic_change +entry.predicted_economic_growth
          yearSum += previousYearJobs5[sector] + (previousYearJobs5[sector] * entry.relationship_variable * (econ / 100));
        }
      }
      ...
      for (const sector of sectors) {
        const sectorData = yearData.filter((entry) => entry.sector_id === sector);
        for (const entry of sectorData) {
          const newGvaPerc = previousYearGVAPerc5[sector] + entry.change / 100;
          const sectorTotalJobs = yearSum * newGvaPerc;
...
//   let employmentAndEconomicChange = updateEmploymentEconomic(employmentData)
  ...
  const handleChange4 = (event, index, column, ) => {
    const value = event.target.value
    setPercentageChange4((currentRows) => {
      const newRow = [...currentRows]
      newRow[index][column] = value
      return newRow
    })
  ...
  const handleChange5 = (event, index, column) => {
    const value = event.target.value
    setPercentageChange5((currentRows) => {
      const newRow = [...currentRows]
      newRow[index][column] = value
      return newRow
    })
  ...
  const datas4 = employmentData4.map((element) => {
    return {
      year: element.year,
      sector_id: element.sector_id,
      total_number_of_jobs: element.total_number_of_jobs,
  ...
  const datas5 = employmentData5.map((element) => {
    return {
      year: element.year,
      sector_id: element.sector_id,
      total_number_of_jobs: element.total_number_of_jobs,
  ...
  return (
                  ...
                  <TableCell align="center">
                    <TextField
                      ...
                      onChange={(event) => {
                        ...
                        handleChange4(event, index, '0', row, 0);
                        handleChange5(event, index, '0', row, 0);
                        }}
                    />
                  </TableCell>
                  ...
                  <TableCell align="center">
                    <TextField
                      ...
                      onChange={(event) => {
                        ...
                        handleChange4(event, index, '1', row, 1)
                        handleChange5(event, index, '1', row, 1)
                      }}
                    />
                  </TableCell>
                  ...
                  <TableCell align="center">
                    <TextField
                      ...
                      onChange={(event) => {
                        ...
                        handleChange4(event, index, '2', row, 2)
                        handleChange5(event, index, '2', row, 2)
                  ...
                  <TableCell align="center">
                    <TextField
                      ...
                      onChange={(event) => {
                        ...
                        handleChange4(event, index, '3', row, 3)
                        handleChange5(event, index, '3', row, 3)
                      }}
                    />
                  </TableCell>
                  ...
                  <TableCell align="center">
                    <TextField
                      ...
                      onChange={(event) => {
                        ...
                        handleChange4(event, index, '4', row, 4)
                        handleChange5(event, index, '4', row, 4)
                      }}
                    />
                  </TableCell>
                  ...
                  <TableCell align="center">
                    <TextField
                      ...
                      onChange={(event) => {
                        ...
                        handleChange4(event, index, '5', row, 5)
                        handleChange5(event, index, '5', row, 5)
                      }}
                    />
                  </TableCell>
                  ...
                  <TableCell align="center">
                    <TextField
                      ...
                      onChange={(event) => {
                        ...
                        handleChange4(event, index, '6', row, 6)
                        handleChange5(event, index, '6', row, 6)
                      }}
                    />
                  </TableCell>
                  ...
                  <TableCell align="center">
                    <TextField
                      ...
                      onChange={(event) => {
                        ...
                        handleChange4(event, index, '7', row, 7)
                        handleChange5(event, index, '7', row, 7)
                      }}
                    />
                  </TableCell>
                  ...
                  <TableCell align="center">
                    <TextField
                      ...
                      onChange={(event) => {
                        ...
                        handleChange4(event, index, '8', row, 8)
                        handleChange5(event, index, '8', row, 8)
                      }}
                    />
                  </TableCell>
                  ...
                  <TableCell align="center">
                    <TextField
                      ...
                      onChange={(event) => {
                        ...
                        handleChange4(event, index, '9', row, 9)
                        handleChange5(event, index, '9', row, 9)
                      }}
                    />
                  </TableCell>
                  ...
                  <TableCell align="center">
                    <TextField
                      ...
                      onChange={(event) => {
                        ...
                        handleChange4(event, index, '10', row, 10)
                        handleChange5(event, index, '10', row, 10)
                      }}
                    />
                  </TableCell>
                </TableRow>
    ...
    <Typography variant="h6" fontWeight="bold">{city1} Employment Projections</Typography>
      ...
      <br></br>
      <Typography variant="h6" fontWeight="bold">{city4} Employment Projections</Typography>
      <TableContainer component={Paper} sx={{ height: '26vh' }}>
        <Table stickyHeader size="small" aria-label="a dense table">
          <TableHead>
            <TableRow>
          ...
          <TableBody>
            {percentageChange4.slice(0, 20).map((row, index) => {
              return (
                <TableRow key={index}>
                  <TableCell align="center" scope="row">
                    <Typography component="div">{row.year}</Typography>
                  ...
                  <TableCell align="center">
                    <TextField
                      ...
                      type="number"
                      value={row['0']}
                      onChange={(event) => handleChange4(event, index, '0', row.year, 0)}
                    />
                  </TableCell>
                  ...
                  <TableCell align="center">
                    <TextField
                      ...
                      type="number"
                      value={row['1']}
                      onChange={(event) => handleChange4(event, index, '1', row.year, 1)}
                    />
                  </TableCell>
                  ...
                  <TableCell align="center">
                    <TextField
                      ...
                      type="number"
                      value={row['2']}
                      onChange={(event) => handleChange4(event, index, '2', row.year, 2)}
                    />
                  </TableCell>
                  ...
                  <TableCell align="center">
                    <TextField
                      ...
                      type="number"
                      value={row['3']}
                      onChange={(event) => handleChange4(event, index, '3', row.year, 3)}
                    />
                  </TableCell>
                  ...
                  <TableCell align="center">
                    <TextField
                      ...
                      type="number"
                      value={row['4']}
                      onChange={(event) => handleChange4(event, index, '4', row.year, 4)}
                    />
                  </TableCell>
                  ...
                  <TableCell align="center">
                    <TextField
                      ...
                      type="number"
                      value={row['5']}
                      onChange={(event) => handleChange4(event, index, '5', row.year, 5)}
                    />
                  </TableCell>
                  ...
                  <TableCell align="center">
                    <TextField
                      ...
                      type="number"
                      value={row['6']}
                      onChange={(event) => handleChange4(event, index, '6', row.year, 6)}
                    />
                  </TableCell>
                  ...
                  <TableCell align="center">
                    <TextField
                      ...
                      type="number"
                      value={row['7']}
                      onChange={(event) => handleChange4(event, index, '7', row.year, 7)}
                    />
                  </TableCell>
                  ...
                  <TableCell align="center">
                    <TextField
                      ...
                      type="number"
                      value={row['8']}
                      onChange={(event) => handleChange4(event, index, '8', row.year, 8)}
                    />
                  </TableCell>
                  ...
                  <TableCell align="center">
                    <TextField
                      ...
                      type="number"
                      value={row['9']}
                      onChange={(event) => handleChange4(event, index, '9', row.year, 9)}
                    />
                  </TableCell>
                  ...
                  <TableCell align="center">
                    <TextField
                      ...
                      type="number"
                      value={row['10']}
                      onChange={(event) => handleChange4(event, index, '10', row.year, 10)}
                    />
                  </TableCell>
                </TableRow>
              )
      ...
      <br></br>
      <Typography variant="h6" fontWeight="bold">{city5} Employment Projections</Typography>
      <TableContainer component={Paper} sx={{ height: '26vh' }}>
        <Table stickyHeader size="small" aria-label="a dense table">
          <TableHead>
            <TableRow>
          ...
          <TableBody>
            {percentageChange5.slice(0, 20).map((row, index) => {
              return (
                <TableRow key={index}>
                  <TableCell align="center" scope="row">
                    <Typography component="div">{row.year}</Typography>
                  ...
                  <TableCell align="center">
                    <TextField
                      ...
                      type="number"
                      value={row['0']}
                      onChange={(event) => handleChange5(event, index, '0', row.year, 0)}
                    />
                  </TableCell>
                  ...
                  <TableCell align="center">
                    <TextField
                      ...
                      type="number"
                      value={row['1']}
                      onChange={(event) => handleChange5(event, index, '1', row.year, 1)}
                    />
                  </TableCell>
                  ...
                  <TableCell align="center">
                    <TextField
                      ...
                      type="number"
                      value={row['2']}
                      onChange={(event) => handleChange5(event, index, '2', row.year, 2)}
                    />
                  </TableCell>
                  ...
                  <TableCell align="center">
                    <TextField
                      ...
                      type="number"
                      value={row['3']}
                      onChange={(event) => handleChange5(event, index, '3', row.year, 3)}
                    />
                  </TableCell>
                  ...
                  <TableCell align="center">
                    <TextField
                      ...
                      type="number"
                      value={row['4']}
                      onChange={(event) => handleChange5(event, index, '4', row.year, 4)}
                    />
                  </TableCell>
                  ...
                  <TableCell align="center">
                    <TextField
                      ...
                      type="number"
                      value={row['5']}
                      onChange={(event) => handleChange5(event, index, '5', row.year, 5)}
                    />
                  </TableCell>
                  ...
                  <TableCell align="center">
                    <TextField
                      ...
                      type="number"
                      value={row['6']}
                      onChange={(event) => handleChange5(event, index, '6', row.year, 6)}
                    />
                  </TableCell>
                  ...
                  <TableCell align="center">
                    <TextField
                      ...
                      type="number"
                      value={row['7']}
                      onChange={(event) => handleChange5(event, index, '7', row.year, 7)}
                    />
                  </TableCell>
                  ...
                  <TableCell align="center">
                    <TextField
                      ...
                      type="number"
                      value={row['8']}
                      onChange={(event) => handleChange5(event, index, '8', row.year, 8)}
                    />
                  </TableCell>
                  ...
                  <TableCell align="center">
                    <TextField
                      ...
                      type="number"
                      value={row['9']}
                      onChange={(event) => handleChange5(event, index, '9', row.year, 9)}
                    />
                  </TableCell>
                  ...
                  <TableCell align="center">
                    <TextField
                      ...
                      type="number"
                      value={row['10']}
                      onChange={(event) => handleChange5(event, index, '10', row.year, 10)}
                    />
                  </TableCell>
                </TableRow>
              )
      ...
      <DialogActions>
        <CsvDownloader
          filename={`gpf_emp_control_totals_ihs`}
          extension=".csv"
          separator=","
          columns={columns}
          datas={datas}
...
export default EmploymentProjectionsGauteng
